...
class learnable_highd_gaussian_continuous(nn.Module):
    def __init__(self,vals,initial_precision = None,max_precision = None,fixed_precision = True, dirichlet=False, n_clients=0):
        ...
        if self.dirichlet:
            self.d_concentration = Parameter(torch.ones(n_clients)*5)
            from torch.distributions.dirichlet import Dirichlet
            self.dist_d = Dirichlet(self.d_concentration)
